...
test("sheet layout retains every field once and revealing senses does not reveal other overview facts", async () => {
  ...
  const published = JSON.stringify(feature.store.data(publicDoc));
  app.drafts.set(publicDoc.id, { text: "Несохранённая заметка", base: "" });
  await app.command({ dataset: { command: "section-toggle", scope: "senses" } });
  assert.ok(app.collapsedSections.has(`${doc.id}:senses`));
  assert.match(app.content(), /eb-mode-facts" hidden>/);
  assert.equal(JSON.stringify(feature.store.data(publicDoc)), published);
  await app.command({ dataset: { command: "section-toggle", scope: "notes" } });
  assert.match(app.content(), /eb-notes-content" hidden>/);
  assert.equal(app.drafts.get(publicDoc.id).text, "Несохранённая заметка");
  await app.command({ dataset: { command: "section-toggle", scope: "senses" } });
  assert.ok(!app.collapsedSections.has(`${doc.id}:senses`));
  game.user = { id: "player", isGM: false };
  ...
  await app.command({ dataset: { command: "section-toggle", scope: "senses" } });
  assert.ok(app.collapsedSections.has(`${publicDoc.id}:senses`));
  assert.equal(JSON.stringify(feature.store.data(publicDoc)), published);
  game.user = game.users.activeGM;
  app.content();
  await app.command({ dataset: { command: "group", scope: "senses" } });
